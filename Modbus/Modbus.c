...
{  
    ModBus_UART_String(Petit_Tx_Buf,Petit_Tx_Buf_Size);
...
void HandleModbusReadHoldingRegisters(void)
{
    ...
    if((Petit_StartAddress+Petit_NumberOfRegisters)>NUMBER_OF_OUTPUT_PETITREGISTERS)
        HandleModbusError(PETIT_ERROR_CODE_02);
    else
    {
        // Initialise the output buffer. The first byte in the buffer says how many registers we have read
        ...
        Petit_Tx_Data.DataBuf[0]  = 10;
        Petit_Tx_Data.DataBuf[1]  = 20;
...
void HandleModbusWriteSingleRegister(void)
{
    ...
    if(Petit_Address>=NUMBER_OF_OUTPUT_PETITREGISTERS)
        HandleModbusError(PETIT_ERROR_CODE_03);
    else
    {
...
void Petit_RxRTU(void)
{
    ...
    Petit_ReceiveBufferControl    =CheckModbusBufferComplete();
...
void ProcessModbus(void)
{
    ...
    if (Petit_RxDataAvailable())                                                // If data is ready enter this!
    {
        if (Petit_Rx_Data.Address == PETITMODBUS_SLAVE_ADDRESS)                 // Is Data for us?
        {
            ...
            {
                #if PETITMODBUS_READ_HOLDING_REGISTERS_ENABLED > 0
                case PETITMODBUS_READ_HOLDING_REGISTERS:    {   HandleModbusReadHoldingRegisters();        break;  }
                #endif
                #if PETITMODBUSWRITE_SINGLE_REGISTER_ENABLED > 0
                case PETITMODBUS_WRITE_SINGLE_REGISTER:     {   HandleModbusWriteSingleRegister();         break;  }
                #endif
                #if PETITMODBUS_WRITE_MULTIPLE_REGISTERS_ENABLED > 0
                case PETITMODBUS_WRITE_MULTIPLE_REGISTERS:  {   HandleMPetitodbusWriteMultipleRegisters();      break;  }
                #endif
                default:                                    {   HandleModbusError(PETIT_ERROR_CODE_01);    break;  }
            }
        }
    }
}
...
{
    PETITMODBUS_SLAVE_ADDRESS    =ModbusSlaveAddress;   
